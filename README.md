...
- **高性能内存池**：三级缓冲区池，减少90%内存分配
- **智能连接池**：连接复用率80%+，连接建立时间减少70%
- **XTLS流控支持**：支持 xtls-rprx-vision 流控模式
...
- **异步处理**: 基于Tokio的异步运行时，支持高并发连接（1000+）
- **内存池优化**: 三级缓冲区池（4KB/64KB/128KB），减少90%内存分配
- **连接池管理**: 智能连接复用，复用率80%+，连接建立时间减少70%
- **零拷贝传输**: 使用Bytes库进行高效的内存管理
- **批量统计**: 64KB批量阈值，减少90%锁竞争
- **可配置缓冲区**: 默认128KB传输缓冲区，支持高带宽场景（千兆网络）
- **TCP优化**: 默认启用TCP_NODELAY降低延迟
- **单文件部署**: 静态资源嵌入可执行文件，约1.2MB，无需额外依赖
